...
const ItemCategory = styled.li`
  ...
  background: ${(props) => (props.isEnabled ? '#000' : '#fff')};
  color: ${(props) => (props.isEnabled ? '#fff' : '#000')};
`;
...
const SubCategory = ({ el, level }) => {
  ...
  const handlerClick = () => {
    setIsEnabled(true);
    CtrlMenuTree.send({ data: el, level });
  };
  useEffect(() => {
    const ctrl = CtrlMenuTree.get().subscribe(async (message) => {
...
const ReactSidemenuTree = ({ dataTree }) => (
  <ContainerCategory>
    <Ul>
      {dataTree.map((el) => (
